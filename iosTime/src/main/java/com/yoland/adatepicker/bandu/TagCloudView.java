...
public class TagCloudView extends ViewGroup {
    ...
    private List<FeedTagBean.DataBean.ListBean> tags;
    ...
    private static final int DEFAULT_TEXT_BACKGROUND = R.drawable.tag_item_nomal;
    ...
    public void setTags(List<FeedTagBean.DataBean.ListBean> tagList) {
        this.tags = tagList;
        this.removeAllViews();
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                tagView.setLayoutParams(layoutParams1);
                tagView.setText(tags.get(i).getTag());
                if (tags.get(i).isSelect()) {
                    tagView.setBackgroundResource(R.drawable.tag_item_selected);
                    tagView.setTextColor(getResources().getColor(R.color.white));
                } else {
                    tagView.setBackgroundResource(R.drawable.tag_item_nomal);
